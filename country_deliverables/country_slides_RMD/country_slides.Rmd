...
## Figure 4 Stability of risk classification across 2011-2015 and 2016-2020
...
::: notes
Figure 4. Stability of risk classification in Africa across 2011-2010
This set of figures aim to show the stability of cholera risk classification of the second-level administrative districts across 2011-2015 and 2016-2020. There are four cholera risk categories, including “High-both” (districts were assigned to a high risk category of >10 per 100,000 in both periods), “High-either” (districts were assigned to a high risk category of >10 per 100,000 in exactly one period), “Low-both” (districts were assigned to a low risk category of <1 per 100,000 in both periods), and “Mix” (districts had a moderate risk category of >1 and <10 per 100,000 in both periods or a moderate and low risk category in one period each).

A) Panel A shows the stability of cholera risk category for districts where we have high confidence in assigning them into high or low risk incidence rate categories across 2011-2015 and 2016-2020 periods.
B) Panel B shows the 2020 population living in second-level administrative districts in a given high confidence stability category across 2011-2015 and 2016-2020 periods.
...
## Figure 9. Population by incidence risk category and time period
...
## Figure 10. Stability of risk classification across 2011-2015 to 2016-2020
...
::: notes
Figure 10. Stability of risk classification in  `r country_full_name` across 2011-2010
This set of figures aim to show the stability of cholera risk classification of the second-level administrative districts across 2011-2015 and 2016-2020 in  `r country_full_name`. There are four cholera risk categories, including “High-both” (districts were assigned to a high risk category of >10 per 100,000 in both periods), “High-either” (districts were assigned to a high risk category of >10 per 100,000 in exactly one period), “Low-both” (districts were assigned to a low risk category of <1 per 100,000 in both periods), and “Mix” (districts had a moderate risk category of >1 and <10 per 100,000 in both periods or a moderate and low risk category in one period each). The color shows the stability of cholera risk category for districts where we have high confidence in assigning them into high or low risk incidence rate categories across 2011-2015 and 2016-2020 periods. The names of first-level administrative units are also shown in the maps.
...
## Table 2. Number of data points by year for `r country_full_name`
...
```{r,  cases by admin unit, echo=FALSE,message=FALSE,warning=FALSE,results='hide'}
cases <- readRDS(paste0(opt$output_dir,"/2011_2015_mai_cases_adm.rds")) %>% 
  bind_rows(readRDS(paste0(opt$output_dir,"/2016_2020_mai_cases_adm.rds"))) %>% 
  subset(country == country_iso) %>%
  distinct()

library(xlsx)

write.xlsx(cases%>% subset(admin_level=="ADM0") %>% mutate(Country = shapeName, `Mean annual incidece` = mean, `The lowest 5 percentile of the mean annual incidence` = q5, `The highest 95 percentile of the mean annual incidence`=q95) %>% select(Country,`Mean annual incidece`,`The lowest 5 percentile of the mean annual incidence`,`The highest 95 percentile of the mean annual incidence`) %>% data.frame(), file=paste0(country_iso,"_mean_annual_incidence.xlsx"), sheetName="Country", row.names=FALSE)

write.xlsx(cases%>% subset(admin_level=="ADM1") %>% mutate(Admin1 = shapeName, `Mean annual incidece` = mean, `The lowest 5 percentile of the mean annual incidence` = q5, `The highest 95 percentile of the mean annual incidence`=q95) %>% select(Admin1,`Mean annual incidece`,`The lowest 5 percentile of the mean annual incidence`,`The highest 95 percentile of the mean annual incidence`) %>% data.frame(), file=paste0(country_iso,"mean_annual_incidence.xlsx"), sheetName="Admin1",  append=TRUE,row.names=FALSE)

cases_adm2<-cases%>%subset(admin_level=="ADM2") %>% mutate(admin1=NA)
cases_adm1<-cases%>%subset(admin_level=="ADM1") %>%
  mutate(location_period_id=stringr::str_remove(location_period_id,paste0(country_iso,"-ADM1-"))) %>% 
  mutate(location_period_id=stringr::str_remove(location_period_id,"_1"))

for (row_idx in 1:nrow(cases_adm1)) {
  cases_adm2[str_detect(cases_adm2$location_period_id,cases_adm1$location_period_id[row_idx]),]$admin1=cases_adm1$shapeName[row_idx]

}

write.xlsx(cases_adm2 %>% mutate(Admin1 = admin1,Admin2=shapeName, `Mean annual incidece` = mean, `The lowest 5 percentile of the mean annual incidence` = q5, `The highest 95 percentile of the mean annual incidence`=q95) %>% select(Admin1,Admin2,`Mean annual incidece`,`The lowest 5 percentile of the mean annual incidence`,`The highest 95 percentile of the mean annual incidence`) %>% data.frame(),file=paste0(country_iso,"_mean_annual_incidence.xlsx"), sheetName="Admin2",  append=TRUE,row.names=FALSE)
