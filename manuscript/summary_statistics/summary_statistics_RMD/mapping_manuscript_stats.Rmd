...
summary_obs_counts_1620 <- output_obs_counts_1620 %>%
  ...
  dplyr::summarise(
    ...
    total_obs = sum(n_obs),
    unique_OCs = length(unique(output_obs_1620$OC_UID)),
    unique_loc = length(unique(location_name)),
    unique_admin_units = length(unique(admin_level)),
    total_national_obs = sum(admin_level==0),
    total_subnational_obs = sum(!admin_level==0),
...
summary_obs_counts_1120 <- output_obs_counts_1120 %>%
  ...
  dplyr::summarise(
    ...
    total_obs = sum(n_obs),
    unique_OCs = length(unique(output_obs_1120$OC_UID)),
    unique_loc = length(unique(location_name)),
    unique_admin_units = length(unique(admin_level)),
    total_national_obs = sum(admin_level==0),
    total_subnational_obs = sum(!admin_level==0),
...
shp_adm2_1620 <- sf::st_make_valid(readRDS(paste0(params$postprocessed_output_directory,'/2011_2015_shapefiles.rds')) %>% dplyr::filter(admin_level >=2))
...
for (country_tmp in unique(shp_adm2_1115[shp_adm2_1620$locationPeriod_id%in%shp_overlap_lp,]$country)) {
...
grid_case_adm0_1115 %>% 
  dplyr::mutate(X..of.grid.cells.with..1.modeled.cases.year = round(X..of.grid.cells.with..1.modeled.cases.year,0)) %>% 
  dplyr::rename("% of grid cells with >1 mean modeled cases/year"=X..of.grid.cells.with..1.modeled.cases.year) %>% 
...
output_endemicity<-readRDS(paste0(params$postprocessed_output_directory,'/endemicity.rds'))
  ...
  output_endemicity %>% 
  dplyr::group_by(country) %>% 
  dplyr::summarise(highrisk_adm2_units = 100 * length(endemicity[endemicity=="high-both"])/n()) %>% 
...
summary_both_high_risk_adm2_1120 <- output_endemicity %>% 
  dplyr::ungroup() %>% 
  dplyr::summarise(bothhighrisk_adm2_units = 100 * length(endemicity[endemicity=="high-both"])/n(),
